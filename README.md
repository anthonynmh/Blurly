...
- **BYOK — Bring Your Own Key.** Blurly never ships a default API key. You enter your own OpenAI key in **AI Settings** → it is stored in your **macOS Keychain** via the Rust `keyring` crate. The key is **never** written to `blurly.db`. Only the provider id (e.g. `openai`) is kept in `ai_settings.key_ref` as a flag.
